...
function SignIn({ onSwitch, onSuccess, supabase }) {
  ...
  // OAUTH-FIX: separate loading state for OAuth buttons.
  // handleOAuth takes 1-2s before the browser redirects. Without a guard,
  // multiple clicks queue overlapping PKCE flows — Supabase overwrites the
  // code_verifier in sessionStorage on each call, so the first redirect's
  // verifier is gone and PKCE verification fails on return.
  ...
  const handleOAuth = async (provider) => {
    if (oauthLoading) return;  // OAUTH-FIX: block re-entry while redirect is pending
    ...
    // Note: on success the browser navigates away, so setOauthLoading(false)
    // only runs when signInWithOAuth resolves with an error.
    if (error) {
  ...
  return (
    <div key="signin" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      {/* Social row — OAUTH-FIX: disabled + opacity during oauthLoading to prevent
           multiple clicks that would overwrite Supabase PKCE code_verifier */}
      <div className="field-enter-1" style={{ display: "flex", gap: 10 }}>
        <button className="social-btn" onClick={() => handleOAuth("google")}
          disabled={oauthLoading}
          style={{ opacity: oauthLoading ? 0.5 : 1, cursor: oauthLoading ? "not-allowed" : "pointer" }}>
...
function SignUp({ onSwitch, onSuccess, supabase }) {
  ...
  // OAUTH-FIX: same guard as SignIn — prevent double-click PKCE nonce corruption
  const [oauthLoading, setOauthLoading] = useState(false);
  const handleOAuth = async (provider) => {
